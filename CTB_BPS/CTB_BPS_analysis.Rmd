...
setwd("~/GitHub/Experiments/CTB_BPS/")
# setwd("~/Library/Mobile\ Documents/com~apple~CloudDocs/time-course/data")

raw_data <- "CTB_pilot_48hr-ExperimentSummary.csv"
# key_data <- "explant_key.csv"
standard_data <- "std_CTB_pilot_48hr-ExperimentSummary.csv"
...
plot_of_48 <- data3 %>% 
 ggplot(aes(x=particle_size, y=tech_mean,color=Injection ))+ #plot
  ...
  xlab("Particle Size") + # X axis label
  ylab("\nMean Particle Concentration/ml\n") + # Y axis label
  ggtitle("Nanosight Histogram of\nHuman CTBs Treated with BPS")+ #title
...
data4 %>%
  ...
  ggplot(aes(x = reorder(Sample_ID,particle_conc), y = particle_conc ))+
  geom_col()
